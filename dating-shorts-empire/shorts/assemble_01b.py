#!/usr/bin/env python3
"""Assemble intro v2 (user-voice edition): one continuous VO track, visual
beats cut at absolute timestamps. BEATS holds (t_end, asset, kind, params,
caption, color) rows — each row runs from the previous row's t_end. Long
spans are auto-split into <=2s sub-beats alternating wide/punch framing."""
...
VO = os.path.join(HERE, "..", "voice-vo", "vo-final.wav")
TMP = os.path.join(HERE, "tmp01b"); os.makedirs(TMP, exist_ok=True)
NAVY, RED, GREEN = a1.NAVY, a1.RED, a1.GREEN

# (t_end, asset, kind, params, caption, color) — filled from whisper segments
BEATS = []  # populated by build_beats() below at run time
...
def render(beats):
    ...
    for i, (t_end, asset, kind, prm, cap, col) in enumerate(beats):
        d = t_end - t_prev
        # auto-split spans > 2.0s into alternating wide/punch sub-beats
        n = max(1, int(-(-d // 1.9)))
        sub = d / n
        for j in range(n):
            ...
            if cap:
                ...
                cp = a1.caption_png(cap, f"01b_{i}_{j}", col)
    ...
    subprocess.run(["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", lst, "-c:v", "libx264",
                   ...
                   check=True, capture_output=True)
    final = os.path.join(HERE, "01-intro-john-voice.mp4")
...
if __name__ == "__main__":
    # BEATS is written into beats_01b.py by the operator after whisper timing
    from beats_01b import BEATS as B
    render(B)
